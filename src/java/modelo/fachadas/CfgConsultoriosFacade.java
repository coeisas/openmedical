...
public class CfgConsultoriosFacade extends AbstractFacade<CfgConsultorios> {
    ...
    public List<Integer> getIdBySede(int id_sede) {
        Query query = getEntityManager().createQuery("SELECT c.idConsultorio FROM CfgConsultorios c WHERE c.idSede.idSede = ?1", Integer.class);
        query.setParameter(1, id_sede);
        return query.getResultList();
    }
